...
class Adapter {
    ...
    connect() {
        this.store.io.on('connection', socket => {
    ...
    hash(data, global) {
        ...
        let socket = global ? this.store.io : this.socket;

        this.newHash = hash;

        socket.emit(this.name + '_hash', hash);
    }

    initEndPoints() {
        let path = this.name;

        this.socket.on(path, (prop, cb) => {
            this.full(cb);
        });

        this.socket.on(path + '_patch', (hash, cb) => {
            console.log(hash);
        ...
        // this.store.server.get(path, (req, res) => {
        //   this.full(res);
        // });
        //
        // this.store.server.get(path + '/patch/:hash', (req, res) => {
        //   let oldHash = req.params.hash;
        //
        //   this.inHistory(oldHash).then(match => {
        //     if (match) {
        //       this.match = match;
        //       this.patch(oldHash, res);
        //     } else {
        //       console.log('we did not make it to the patch');
        //       this.full(res);
        //     }
